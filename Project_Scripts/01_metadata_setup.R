...
# SCRIPT:  01_metadata_setup.R
# AUTHOR:  Corona Management Systems
# DATE:    November 26, 2025
#
# PURPOSE:
# Sets up the metadata used across the survey pipeline.
#
# This script defines:
# 1. raw-to-standard column names,
# 2. questionnaire-based answer options,
# 3. variable labels, and
# 4. value labels for recoded data.
#
# OUTPUT:
#   - Sproxil_dictionary.rds
#
# REPLICATION NOTE:
# This file serves as the central metadata reference for downstream cleaning,
# recoding, labelling, and documentation scripts.
# ==============================================================================
...
# ------------------------------------------------------------------------------
# 1. Raw-to-standard column mapping
# ------------------------------------------------------------------------------
# Maps exported survey field names to the standard names used in the analysis.
column_mapping <- c(
  "meta_status"                     = "status",
...
# ------------------------------------------------------------------------------
# 2. Questionnaire answer dictionary
# ------------------------------------------------------------------------------
# Stores the expected raw response options based on the survey instrument.
...
# ------------------------------------------------------------------------------
# Add repeated asset variables
# ------------------------------------------------------------------------------
# Applies the same answer options to all asset ownership variables.
...
# ------------------------------------------------------------------------------
# Add livestock and agricultural count variables
# ------------------------------------------------------------------------------
# Applies the same count structure to livestock and land variables.
...
# 3. Variable labels
# ------------------------------------------------------------------------------
# Provides short descriptive labels for variables used in outputs and documentation.
variable_labels <- c(
  ...
  "msg_dontremember"      = "Msg Source: I don't Know",
  "msg_other"      = "Msg Source: Others",
  ...
  # --- Derived stratifiers and indices ---
  ...
  # --- Derived indicators: nets and ITNs ---
  ...
  "derived_num_itns"                = "Minimum confirmed number of ITNs in household (proxy: 1 if household has at least one ITN, 0 otherwise)",
  "derived_access_itn"              = "Minimum ITN access proxy (at least one confirmed ITN per two usual household members)",
  ...
  # --- Derived indicators: ANC and IPTp ---
  ...
  # --- ACT timing and effectiveness: among ACT takers ---
  ...
  "hh_size_num"                     = "Total number of usual household members in the household",
  "ppl_slept_net_num"               = "Number of people who slept under the mosquito net last night",
  
  # --- Additional derived variables, weighting helpers, and top-code flags ---
  ...
  "pca_floor_improved"     = "PCA helper: improved floor material indicator",
  "pca_roof_improved"      = "PCA helper: improved roof material indicator",
  "pca_wall_improved"      = "PCA helper: improved wall material indicator",
  "pca_water_improved"     = "PCA helper: improved drinking water source indicator",
  "pca_sanitation_improved"= "PCA helper: improved sanitation facility indicator",
  "pca_toilet_not_shared"  = "PCA helper: toilet facility not shared indicator",
  "pca_clean_cooking"      = "PCA helper: clean cooking technology indicator",
  "pca_sleeping_rooms_num" = "PCA helper: number of sleeping rooms",
  "pca_hh_size_num"        = "PCA helper: household size using usual members",
  "pca_low_crowding"       = "PCA helper: low household crowding indicator",
  "pca_complete_case"      = "PCA helper: complete case used in wealth PCA"
)

# ------------------------------------------------------------------------------
# 4. Value labels
# ------------------------------------------------------------------------------
# Defines numeric value labels used after recoding.
value_labels <- list(
  # --- Common scales and binary labels ---
  yesno = c("No" = 0, "Yes" = 1, "Don't Know" = 8, "Missing" = 9),
  
  # Standard labels for binary derived variables
  ...
  # --- SMC, vaccine, and feedback variables ---
  ...
  fuel_type = c("Alcohol" = 1, "Gasoline" = 2, "Kerosene" = 3, "Coal/Lignite" = 4,
                "Charcoal" = 5, "Wood" = 6, "Straw/Shrubs/Grass" = 7, "Agri Crop" = 8,
                "Animal Dung" = 9, "Biomass" = 10, "Garbage" = 11, "Sawdust" = 12,
                "Other" = 96, "Missing" = 99),
  ...
  # Uses list-based labels for derived analysis variables.
  zone = c(
  ...
  age_group_women = c(
    ...
    "35-39" = 5, "40-44" = 6, "45-49" = 7, "Undisclosed" = 8,
    "Missing" = 9
  ),
...
# ------------------------------------------------------------------------------
# 5. Save metadata objects
# ------------------------------------------------------------------------------
# Combines all metadata objects and saves them for reuse in later scripts.
