...
# Helper Function: Ensures NA result in NA dummies
...
df_dummies <- df_recode %>%
  mutate(
    ...
    anc_prov_nurse = dummify(women_anc_provider, "NURSE"),
...
df_recoded <- df_dummies %>%
  mutate(
    ...
    hh_drinking_water_source = case_when(
      ...
      str_detect(hh_drinking_water_source, "UNPROTECTED WELL|DUG WELL") ~ 32, # Check UN- first
      str_detect(hh_drinking_water_source, "PROTECTED WELL") ~ 31,
      str_detect(hh_drinking_water_source, "UNPROTECTED SPRING") ~ 42,        # Check UN- first
    ...
    # --- L. PREGNANCY TIMING (CONSOLIDATION TO MONTHS) ---
...
val_labels(df_labelled$urban_status)       <- value_labels$urban_status
...
val_labels(df_labelled$women_anc_first_visit_month)     <- value_labels$months_special
val_labels(df_labelled$women_pregnancy_duration_months) <- value_labels$months_special
