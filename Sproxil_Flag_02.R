...
# -------------------------------------------------------------------
# 6. LOGICAL CONSISTENCY RULES - CORRECTED
# -------------------------------------------------------------------
...
flag_df <- flag_df %>%
  mutate(
    ...
    # We check if ANY of the key women's variables are not NA
    flag_demo_gender_wqskip = ifelse(
      (!is.na(demo_gender) & demo_gender == "MALE") &
        (!is.na(women_ever_given_birth) | !is.na(women_anc_seen) | !is.na(women_currently_pregnant)), 
      1, 0),
    
    # Says "No Education" but filled "Informal Education" details
    flag_demo_edulevel_informal = ifelse(
      (!is.na(demo_edu_level) & demo_edu_level == "I NEVER HAD ANY FORMAL EDUCATION") &
        !is.na(demo_edu_informal) & demo_edu_informal != "I NEVER HAD ANY FORMAL EDUCATION", 
      1, 0)
  )
...
flag_df <- flag_df %>%
  mutate(
    ...
    flag_prev_hasnets_logic = ifelse(
      (!is.na(prev_has_mosquito_nets) & prev_has_mosquito_nets == "NO") &
        (!is.na(prev_num_mosquito_nets) | !is.na(prev_net_brand) | !is.na(prev_net_obtained_how)), 
      1, 0)
  )
...
flag_df <- flag_df %>%
  mutate(
    ...
    flag_treat_fever_logic = ifelse(
      (!is.na(treat_hh_fever_last_2weeks) & treat_hh_fever_last_2weeks == "NO") &
        (!is.na(treat_blood_sample_taken) | !is.na(treat_drug_cost)), 
      1, 0),
    
    # Says NO Blood Sample, but filled test cost
    flag_treat_bsample_logic = ifelse(
      (!is.na(treat_blood_sample_taken) & treat_blood_sample_taken == "NO") &
        !is.na(treat_test_cost), 
      1, 0)
  )

## 4. GOVERNMENT CONTROL / SMC
flag_df <- flag_df %>%
  mutate(
    # Heard SMC = NO, but says children received it
    flag_treat_smc_heard_logic = ifelse(
      (!is.na(treat_heard_smc) & treat_heard_smc == "NO") &
        (!is.na(treat_children_received_smc) | !is.na(treat_know_smc_drug)), 
      1, 0),
    
    # Children Received SMC = NO (or don't have kids), but knows the drug name
    flag_treat_smc_rec_logic = ifelse(
      (!is.na(treat_children_received_smc) & str_detect(treat_children_received_smc, "^NO")) &
        !is.na(treat_know_smc_drug), 
      1, 0),
    
    # Doesn't know vaccine age, but says children received it
    flag_treat_vaccine_logic = ifelse(
      (!is.na(treat_vaccine_age_knowledge) & str_detect(treat_vaccine_age_knowledge, "I DON'T KNOW")) &
        (!is.na(treat_children_received_vaccine) & treat_children_received_vaccine == "YES"), 
      1, 0)
  )
...
flag_df <- flag_df %>%
  mutate(
    # Says Never Given Birth, but filled children count or ANC
    flag_women_birth_logic = ifelse(
      (!is.na(women_ever_given_birth) & women_ever_given_birth == "NO") &
        (!is.na(women_births_2020_2025) | !is.na(women_anc_seen)), 
      1, 0),
    
    # Says NO ANC, but filled provider/location
    flag_women_anc_logic = ifelse(
      (!is.na(women_anc_seen) & women_anc_seen == "NO") &
        (!is.na(women_anc_provider) | !is.na(women_anc_location)), 
      1, 0),
    
    # Says NO SP/Fansidar, but filled doses
    flag_women_sp_logic = ifelse(
      (!is.na(women_took_sp_fansidar) & women_took_sp_fansidar == "NO") &
        (!is.na(women_sp_fansidar_doses) | !is.na(women_sp_fansidar_source)), 
      1, 0),
    
    # Says Child NO Fever, but filled diagnosis
    flag_women_childfever_logic = ifelse(
      (!is.na(women_child_fever_2weeks) & women_child_fever_2weeks == "NO") &
        (!is.na(women_child_blood_sample) | !is.na(women_child_malaria_diagnosis)), 
      1, 0),
    
    # Says Child Took NO Medicine, but filled medicine type
    flag_women_childmed_logic = ifelse(
      (!is.na(women_child_took_medicine) & women_child_took_medicine == "NO") &
        !is.na(women_child_medicine_type), 
      1, 0),
    
    # Says Not Pregnant, but filled duration
    flag_women_pregnant_logic = ifelse(
      (!is.na(women_currently_pregnant) & women_currently_pregnant == "NO") &
        !is.na(women_pregnancy_duration_months), 
      1, 0)
  )
...
flag_df <- flag_df %>%
  mutate(
    ...
    flag_bg_internet_logic = ifelse(
      (!is.na(bg_internet_ever_used) & bg_internet_ever_used == "NO") &
        !is.na(bg_internet_frequency), 
      1, 0),
    
    # Says Never Heard Malaria Msg, but filled source
    flag_bg_malmsg_logic = ifelse(
      (!is.na(bg_heard_malaria_msg_6months) & bg_heard_malaria_msg_6months == "NO") &
        !is.na(bg_malaria_msg_source), 
      1, 0),
    
    # Says Not Aware of Avoidance, but listed methods
    flag_bg_malavoid_logic = ifelse(
      (!is.na(bg_aware_avoidance) & bg_aware_avoidance == "NO") &
        !is.na(bg_prevention_knowledge), 
      1, 0)
  )
...
flag_df <- flag_df %>%
  mutate(
    ...
    flag_hh_toilet_logic = ifelse(
      (!is.na(hh_toilet_type) & str_detect(hh_toilet_type, "NO FACILITY")) &
        (!is.na(hh_toilet_shared) | !is.na(hh_toilet_location)), 
      1, 0),
    
    # Says No Livestock, but filled counts
    flag_hh_livestock_logic = ifelse(
      (!is.na(hh_owns_livestock) & hh_owns_livestock == "NO") &
        (!is.na(hh_num_cows_bulls) | !is.na(hh_num_goats) | !is.na(hh_num_poultry)), 
      1, 0),
    
    # Says No Agri Land, but filled plots
    flag_hh_agriland_logic = ifelse(
      (!is.na(hh_owns_agri_land) & hh_owns_agri_land == "NO") &
        !is.na(hh_num_agri_plots), 
      1, 0),
    
    # Logic: Children under 5 > Total Persons in Household
    # Ensure numeric conversion happens safely
    hh_u5_numeric = as.numeric(as.character(hh_members_under5)),
    hh_total_numeric = as.numeric(as.character(hh_total_persons_v1)),
    
    flag_hh_children_vs_total = ifelse(
      !is.na(hh_u5_numeric) & !is.na(hh_total_numeric) & hh_u5_numeric > hh_total_numeric, 
      1, 0)
  ) %>%
